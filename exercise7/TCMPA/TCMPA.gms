...
SETS t0    Extended time periods / t0*t1 /
SETS t(t0) Time periods / t1 /
...
PARAMETERS
pg_max(g) Max. active power generation [MW] /G1 10/
pg_min(g) Min. active power generation [MW] /G1  0/
ru(g)     Ramp-up limit [MW:h]              /G1  10/
rd(g)     Ramp-down limit [MW]              /G1  10/
p0(g)     Initial power output [MW]         /G1 0/;
...
SET  b      Buses            / bus1*bus2 /;
SET refb(b) Reference bus    / bus1 /;
alias(b,b_o,b_d);

SET gb(g,b) Generation buses / G1.bus1 /;
SET db(d,b) Demand buses     / D1.bus2 /;

* Lines;

SET l(b_o,b_d) lines / bus1.bus2 /;
