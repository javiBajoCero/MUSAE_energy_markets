...
% 
% Script to plot the market clearing from the GAMS files or the MPA model.
% The input files are created by the MPA2M.gms GAMS script.
%
...
lbgAcc = importdata('MPA_lbg.dat');
pbgAcc = importdata('MPA_pbg.dat');
PGAcc = importdata('MPA_PG.dat');
...
lbdAcc = importdata('MPA_lbd.dat');
pbdAcc = importdata('MPA_pbd.dat');
PDAcc = importdata('MPA_PD.dat');
...
for t=1:tp
    ...
    SW = MPA_eq(3,t);
    SWT = SWT + SW;
    subplot(ceil(tp/ncol),ncol-1,t)
    hold on;
    
    box on;
    title({['\fontname{arial}{\bf Time period }',num2str(t)];['{\it\lambda}^*=',num2str(price),', {\itP }^*=',num2str(energy),', {\itSW ^*}= ',num2str(SW)]},'FontName','Cambria Math','Fontsize',20,'FontWeight','bold');
    ylabel('{\it\lambda} [€/MWh]','Fontname','Cambria Math','Fontsize',20)
    xlabel('{\itP} [MW]','Fontname','Cambria Math','Fontsize',20)
    set(gcf,'Color','w'); set(gca,'Fontsize',20);
    maxenergy = max(xg(nbg+2,t),xd(nbd+2,t))+1;
    maxprice = max(yg(nbg+2,t),yd(1,t))+1;
    axis ([0 maxenergy 0 maxprice]);
    stairs(xg(:,t),yg(:,t),'-','Color',[.26,.26,0.8],'LineWidth',2)
    stairs(xmg(:,t),ymg(:,t),'-','Color',[.13,.13,0.4],'LineWidth',8)
    stairs(xd(:,t),yd(:,t),'-','Color',[.6,.6,.0],'LineWidth',2)
    stairs(xmd(:,t),ymd(:,t),'-','Color',[0.3,.3,.0],'LineWidth',8)
...
subplot (ceil(tp/ncol),ncol-1,tp-1);
text(maxenergy/2, -6, ['\bf{\it SW ^*} = ',num2str(SWT)],'Fontname','Cambria Math','Fontsize',20);
